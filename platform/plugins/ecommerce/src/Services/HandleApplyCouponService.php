...
class HandleApplyCouponService
{
    ...
    public function applyCouponWhenCreatingOrderFromAdmin(Request $request): array
    {
        ...
        if (in_array($discount->target, ['customer', 'once-per-customer'])) {
            if ($customerId = $request->input('customer_id')) {
                if ($discount->target == 'customer' && ! $discount->customers()->where('customer_id', $customerId)->count()) {
                    return [
                        'error' => true,
                        'message' => trans('plugins/ecommerce::discount.invalid_coupon'),
                    ];
                ...
                if ($discount->target == 'once-per-customer' && $discount->usedByCustomers()->where('customer_id', $customerId)->count()) {
                    return [
                        'error' => true,
                        'message' => trans('plugins/ecommerce::discount.customer_used_coupon_code'),
                    ];
                }
            } else {
                return [
